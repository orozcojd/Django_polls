...
class QuestionMethodTests(TestCase):
	...
	def test_was_published_recently_with_recent_question(self):
		...
		time = timezone.now() - dateime.timedelta(hours = 1)
	...
	def create_question(question_text, days):
		'''
			...
			text and published the given number of days offset to now
		'''
...
class QuestionViewTests(TestCase):
	...
	def test_index_view_with_a_future_question(self):
		'''
			questions with a pub_date in the future shouldn't be displayed on
			the index page
		'''
		create_question(question_text = "Future question.", days = 30)
		response = self.client.get(reverse('polls:index'))
		self.assertContains(response, "No polls are available.")
		self.assertQuerysetEqual(response.context['latest_question_on_list'], [])
	...
	def test_index_vew_with_two_past_questions(self):
		'''
